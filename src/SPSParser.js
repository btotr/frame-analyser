var parseSPS = (function(){
	...
	Bitstream.prototype.read32 = function(){
		var byte = "";
	    for (var i=0;i<32;i++) {
	    	byte += ""+this.readBit();
	    }
	...
	Bitstream.prototype.read16 = function(){
		var byte = "";
	    for (var i=0;i<16;i++) {
	    	byte += ""+this.readBit();
	    }
	...
	return function parseSPS(nal){
		...
		vui_parameters_present_flag = stream.readBit();
		if(vui_parameters_present_flag) {
		    vui_parameters = {};
		    vui_parameters.aspect_ratio_info_present_flag = stream.readBit();
           if (vui_parameters.aspect_ratio_info_present_flag) {
                vui_parameters.aspect_ratio_idc = stream.readByte();
                if (vui_parameters.aspect_ratio_idc == 255) {
            		vui_parameters.sar_width = stream.read16();
            		vui_parameters.sar_height = stream.read16();
        		}
           }
           
        	vui_parameters.overscan_info_present_flag = stream.readBit();
            vui_parameters.video_signal_type_present_flag = stream.readBit();
            vui_parameters.chroma_location_info_present_flag = stream.readBit();
			vui_parameters.timing_info_present_flag = stream.readBit();
			
            if (vui_parameters.timing_info_present_flag) {
                vui_parameters.num_units_in_tick = stream.read32();
                vui_parameters.time_scale = stream.read32();
                vui_parameters.fixed_frame_rate_flag = stream.readBit();
            }
		}
		return {
