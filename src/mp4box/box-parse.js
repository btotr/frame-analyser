...
BoxParser.avcCBox.prototype.parse = function(stream) {
	...
	for (i = 0; i < nb_nalus; i++) {
		...
		// FIXME should propagate to info readystat
		this.nal = parseSPS(this.SPS[i]);
		window.fps = this.nal.vui_parameters.time_scale / (2*this.nal.vui_parameters.num_units_in_tick);
		this.size -= 2+length;
	}
...
BoxParser.sidxBox.prototype.parse = function(stream) {
	...
	for (var i = 0; i < count; i++) {
		...
		ref.reference_size = tmp_32 & 0x7FFFFFFF;
